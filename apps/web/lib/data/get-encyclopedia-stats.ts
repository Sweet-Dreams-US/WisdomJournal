...
export async function getEncyclopediaStats(): Promise<EncyclopediaStats | null> {
  try {
    ...
    const { data, error } = await supabase.rpc("get_encyclopedia_stats", {
      p_user_id: user.id,
    });

    if (error) {
      console.error("getEncyclopediaStats RPC error:", error);
      return null;
    }
    ...
    console.error("getEncyclopediaStats error:", error);
    return null;
  }
}
